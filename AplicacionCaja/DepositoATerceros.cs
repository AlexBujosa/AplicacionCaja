...
namespace AplicacionCaja
{
    public partial class DepositoATerceros : Form
    {
        ...
        private void DepositoATerceros_Load(object sender, EventArgs e)
        {
            ...
            valor = comboBox1.Items[0].ToString();
        }

        private void timer1_Tick(object sender, EventArgs e)
        {
            if (!comboBox1.DroppedDown && char.IsDigit(valor[0]))
            {
                int NoCuenta = int.Parse(comboBox1.SelectedItem.ToString());
                foreach(DataRow row in data.Tables[0].Rows)
                {
                    if (int.Parse(row[0].ToString()) == NoCuenta)
                    {
                        label2.Text = "NoCuenta: " + row[0].ToString();
                        label3.Text = "Sexo: " + row[1].ToString();
                        label4.Text = "Cedula: " + row[2].ToString();
                        label5.Text = "Nombres: " + row[3].ToString();
                    }
                }
